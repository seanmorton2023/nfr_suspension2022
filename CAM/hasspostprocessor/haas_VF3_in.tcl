########################## TCL Event Handlers ##########################
#
#  haas_VF3_in.tcl - 3_axis_mill
#
#    
#
#  Created by Admin @ Tuesday, February 20, 2018 3:09:58 PM Central Standard Time
...
#
  ...
  set mom_sys_alt_unit_post_name                "haas_VF3_in__MM.pui"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_tool_number_max                   "20"
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_max_arc_radius                    "99.999"
...
#=============================================================
   MOM_force Once G_mode G_return Z
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_adjust G G_motion G_mode X Y Z H M_coolant}
   set rapid_spindle_x_blk {G_adjust G G_motion G_mode X H M_coolant}
   set rapid_spindle_y_blk {G_adjust G G_motion G_mode Y H M_coolant}
   set rapid_spindle_z_blk {G_adjust G G_motion G_mode Z H M_coolant}
   set rapid_traverse_blk {G G_motion G_mode X Y Z S M_spindle}
   set rapid_traverse_xy_blk {G G_motion G_mode X Y S M_spindle}
   set rapid_traverse_yz_blk {G G_motion G_mode Y Z S M_spindle}
   set rapid_traverse_xz_blk {G G_motion G_mode X Z S M_spindle}
...
#=============================================================
   ...
   PB_CMD_tool_change_force_addresses
   MOM_do_template coolant_off
   MOM_force Once G G_mode Z
   MOM_do_template tool_change
   PB_CMD_deferred_park_move
   PB_CMD_announce_tool
   MOM_force Once T M
   MOM_do_template tool_change_1
...
# PB_CMD_announce_cycle_time
#
#  Machine time for current output
#
# Bind wherever you want to see accumulated time
# -------------------------------------------------------

global mom_machine_time        ;# total time, in minutes
global mom_cutting_time        ;# cutting time, in minutes

# set up some locals so that control OUT and IN can be
...
set intime $mom_machine_time    ;# adjust to suit

set hrs  [expr floor($intime/60)]
set mins [expr floor($intime-($hrs*60))]
set secs [expr round(60*($intime-($hrs*60)-$mins))]

# format time as HH:MM:SS
set tstg [format "%02.0f:%02.0f:%02.0f" $hrs $mins $secs]
MOM_output_text "${co}MACHINE TIME = $tstg${ci}"
}
...
if {![string match "*CONTROL" [string toupper $mom_operation_type]]} {
  ...
  # output to tape
  # - use MOM_output_literal for sequence number,
  # - use MOM_output_text for none
...
global mom_tool_description    ;# NX4 and later
global mom_tool_type        ;# tool template name
...
# handle pre-NX4 possibility of no "mom_tool_description"
if { ![info exists mom_tool_description] } {
  if { $mom_tool_catalog_number!="" } {
    set desc [string toupper $mom_tool_catalog_number]
  } else {
    set desc "-NONE-"
  }
} else { ;# NX4 and later: if DESC is blank, we get template name
  if { $mom_tool_description == $mom_tool_type } {
    set desc $mom_tool_name
  } else {
    set desc $mom_tool_description
  }
}

set desc [string toupper $desc]
#if { $desc == "" } { set desc "NO DESCRIPTION" }

# output to PTP
# - use MOM_output_literal for sequence number,
# - use MOM_output_text for none
set tlmsg "TOOL #$mom_tool_number: $desc"

# for sequence number, use MOM_output_literal
# for none,            use MOM_output_text
#MOM_output_literal "${co}$tlmsg${ci}"
#MOM_output_text "${co}$tlmsg${ci}"

# for later output, comment/delete both of the above,
# and output this GLOBAL as required

global my_tool_name    ;# visible for later output
set my_tool_name $desc
}
...
#
# invoke from START_OF_PATH event
...
#=============================================================
proc PB_CMD_deferred_park_move { } {
#=============================================================
# process buffered park request
# bind this to the TOOL CHANGE event

global mom_park_msg         ;# optional text
global mom_park_msg_defined ;# 0 or 1

global my_park_flag

if { ![info exists my_park_flag] } {
  set my_park_flag 0
}

if { $my_park_flag } {
  if { $mom_park_msg_defined } {
    MOM_do_template park_msg
  }
  MOM_force once M
  MOM_do_template park_move
}
...
#=============================================================
proc PB_CMD_install_MOM_park { } {
#=============================================================
uplevel #0 {
#
# bind this to START_OF_PROGRAM event

#=============================================================
proc MOM_park { } {
#=============================================================
# handler for custom UDE - park machine/program stop/message

global mom_park_mode        ;# IMMEDIATE or DEFERRED
global mom_park_msg         ;# optional text
global mom_park_msg_defined ;# 0 or 1

global my_park_flag

if { $mom_park_mode == "IMMEDIATE" } {
  if { $mom_park_msg_defined } {
    MOM_do_template park_msg
  }
  MOM_do_template park_move
} else {
  # set flag for deferred processing at tool change
  set my_park_flag 1
}
...
set MY_filnam "$prgid.TXT"

MOM_output_text "%"
MOM_output_text "O0001"
MOM_output_text "${co}$prgid - $dt${ci}"
}
